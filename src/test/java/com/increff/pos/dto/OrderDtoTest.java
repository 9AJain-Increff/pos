...
import static com.increff.pos.util.ConversionUtil.convertToOrderItemData;
...
public class OrderDtoTest extends AbstractUnitTest {
    ...
    public void createOrder() throws ApiException {
        ...
        OrderData expectedOrder = orderDto.addOrder(orderItemForms);

        Integer orderId = expectedOrder.getId();
        OrderPojo actualOrder = orderService.getOrderByOrderId(orderId);
        OrderData actualOrderData = convertToOrderData(actualOrder);
        AssertUtil.assertEqualOrderData(expectedOrder, actualOrderData);
    ...
    @Test
    public void getAll() {
        List<OrderData> actual = orderDto.getAllOrders();
    ...
    public void testGetOrderItemsById() throws ApiException {
        OrderPojo expectedOrder = orderList.get(0).getKey();
        List<OrderItemPojo> expectedOrderItems= orderList.get(0).getValue();
        List<OrderItemData> expectedOrderItemsData = new ArrayList<>();
        List<OrderItemData> actualOrderItems = orderDto.getOrderItemsById(expectedOrder.getId());
        for(OrderItemPojo orderItemPojo: expectedOrderItems){
            ProductPojo product = productService.getProductById(orderItemPojo.getProductId());
            expectedOrderItemsData.add(convertToOrderItemData(orderItemPojo,product,expectedOrder.getId()));
        }
        AssertUtil.assertEqualList(expectedOrderItemsData, actualOrderItems,AssertUtil::assertEqualOrderItemsData);
    }
    //    @Test
...
//        }).collect(Collectors.toList());
//    }
    private List<OrderItemData> getOrderItemData(List<OrderItemPojo> orderItems) {
        return orderItems.stream().map(orderItem -> {
            ProductPojo product = productList
                    .stream()
                    .filter(it -> it.getId().equals(orderItem.getProductId()))
                    .findFirst()
                    .orElse(null);
            assert product != null;
            return convertToOrderItemData(orderItem, product, orderItem.getOrderId());
        }).collect(Collectors.toList());
    }
//    @Test
//    public void testGetOrderItemsById() throws ApiException {
